...
jQuery(function(){
	...
	initLazyLoad();
	initCartLink();
	initSelectAfterSearch();
	//initInfiniteScroll();
	//initLoadMore();
});

//initSelectAfterSearch
function initSelectAfterSearch() {
	...
	var selectVolt = jQuery('#pa_voltage');
	var selectPlug = jQuery('#pa_plug');
	
	if (!selectVolt.length || !selectPlug.length) {
		return;
	}
	
	var pa_voltage = getUrlAttr('pa_voltage');
	var pa_plug = getUrlAttr('pa_plug');
	
	if (pa_voltage && pa_plug) {
		selectVolt[0].selectedIndex = 0;
		selectPlug[0].selectedIndex = 0;
		
		selectVolt.val(pa_voltage);
		selectPlug.val(pa_plug);
	}
}
// init cart links
function initCartLink(){
	var link = jQuery('.order-block .btn-cart');
	var tab = jQuery('.step-list li:first .holder');
	if (link.length && tab.length) {
		tab.wrapInner('<a href="'+link.attr('href')+'"></a>');
		link.remove();
	}
}
...
function initTooltip() {
	jQuery('.tooltip-opener').hoverTooltip({
		attribute: 'title',
		positionTypeX: 'right',
...
function initCustomHover() {
	jQuery('.button-disabled').touchHover();
}

// init lazy load
function initLazyLoad(){
	var holder = jQuery('.filter-results');
	ResponsiveHelper.addRange({
		'768..': {
			...
			off: function() {
				// destroy lazy load
				if (holder.data('LazyLoad')) {
					holder.data('LazyLoad').destroy();
				}
			}
		}
	});
...
;(function($){
	...
	LazyLoad.prototype = {
		...
		findElements: function(){
			...
			this.pager = this.hold.find(this.options.pager);
		...
		attachEvents: function(){
			var self = this;
			this.bindHandlers(['onWindowScroll']);
			this.win.on('scroll resize orientationchange', this.onWindowScroll);
		},
		...
		checkPosition: function(){
			var self = this;
			if (this.win.height() + this.win.scrollTop() > this.set.outerHeight() + this.hold.offset().top && this.hiddenLink.length && !this.busy){
		...
		loadItems: function(href){
			...
			jQuery.ajax({
				...
				success: function(msg){
					...
					var newPager = data[self.options.filter ? 'filter' : 'find'](self.options.pager);
					
					newItems.insertBefore(self.pager);
					self.hiddenLink = data.find(self.options.hiddenLink);
					self.hold.removeClass(self.options.loadingClass);
					self.busy = false;
					if (newPager.length) {
						self.pager.replaceWith(newPager);
						self.pager = newPager;
					}
					if (!self.hiddenLink.length) {
						self.hold.addClass('no-content');
					}
		...
		destroy: function(){
			this.win.off('scroll resize orientationchange', this.onWindowScroll);
		}
	};
...
function initLightbox() {
	...
	jQuery('a.iframe-lightbox').each(function(){
		var link = jQuery(this);
		var iframeWidth = +link.attr('data-width') || 560;
		var iframeHeight = +link.attr('data-height') || 340;
		...
		function resizeLightbox(){
			var iframe = jQuery('#fancybox-frame');
			var fancyboxWrap = jQuery('#fancybox-wrap');
			var fancyboxContent = jQuery('#fancybox-content');
			var lightboxPaddings = (fancyboxWrap.outerHeight() - fancyboxWrap.height()) + (fancyboxContent.outerHeight() - fancyboxContent.height());
			
			iframe.width(iframeWidth);
			iframe.height(iframeHeight);
			
			if(win.width()< 768){
				iframe.width(win.width()-lightboxPaddings);
				var currentWidth = iframe.width();
				
				iframe.height(currentWidth/ratio);
			}
...
function initVideoSidebar() {
	var holder = jQuery('.ytcplayer-fixwidthwrapper');
	var videoWrap = holder.find('.ytcplayer-wrapper').wrap('<div class="video-wrap"></div>');
	var sidebar = jQuery('#video-sidebar').appendTo(holder);

	jQuery(holder).sameHeight({
		elements: '.video-wrap, #video-sidebar',
		flexible: true,
		multiLine: true
	});
...
function initSrcollPage(){
	jQuery('a.btn-top').bind('click touchstart', function(){
...
function initSlideShow() {
	jQuery('div.gallery-holder').fadeGallery({
		...
		btnPrev: 'a.btn-prev1',
		btnNext: 'a.btn-next1',
		pagerLinks: '.gallery .slide-holder',
		event: 'click',
...
function initSameHeight() {
	...
	jQuery('.contacts-section').sameHeight({
		elements: '.same-height',
		multiLine:true,
		flexible: true
	});
	// jQuery('.video-section').sameHeight({
	// 	elements: '.same-height',
	// 	multiLine:true,
	// 	flexible: true
	// });
	jQuery('.videos-list').sameHeight({
		elements: '.video-thumbnail',
		multiLine:true,
		flexible: true
	});
	jQuery('#online-dealers').sameHeight({
		elements: '.holder',
		multiLine:true,
		flexible: true
	});
...
function initGenerationStep(){
	var activeClass = 'active';
	var allowClass = 'allow';
	var selectedClass = 'selected';
	var hiddenClass = 'hidden';
	jQuery('.build-section').each(function(){
		var holder = jQuery(this);
		var stepContent = holder.find('.step-content');
		var stepList = holder.find('.step-list');
		var stepTarget = holder.data('target');
		...
		var warning = function(){
			if (activeIndex > 0 && activeIndex != holder.find('.build-block').length-1) {
					return "Are you sure you want to navigate away from this page? \n" + 
							"Any choices you have made will not be saved.";
			}
		}
		window.onbeforeunload = warning;
		
		var initSteps = function(getIndex, sameIndex){
			var steps = holder.find('.step-list li');
			var ajaxHolder = holder.find('.ajax-holder');
			var stepsContent = holder.find('.build-block');
			var links = holder.find('.option-list a');
			var target = steps.eq(steps.length-1).find('a').attr('href');
						
			if (getIndex) {
				var prevIndex = stepsContent.index(stepsContent.filter('[id="'+activeBlock+'"]'));
				activeIndex = stepsContent.index(stepsContent.eq(prevIndex).nextAll().not('.'+hiddenClass).eq(0));
				
				if (sameIndex) {
					activeIndex = prevIndex;
				}
				
				if (activeIndex == stepsContent.length-1 || activeIndex == -1) {
					jQuery.ajax({
						url: target,
						type: 'post',
						data: 'jsonData=' + JSON.stringify(dataStep),
						success: function(result){
							// insert ajax content
							ajaxHolder.html(result);
							ajaxHolder.css({background:'none'});
							initLightbox();

							// set options
							var form = ajaxHolder.find('.variations_form').hide();
							...
							var button = ajaxHolder.find('.add_to_cart_button.product_type_variable');
							var selectVolt = ajaxHolder.find('#pa_voltage');
							var selectPlug = ajaxHolder.find('#pa_plug');
							var nameVolt = selectPlug.attr('name');
							var namePlug = selectPlug.attr('name');
							var buttonSrc = button.attr('href');
							var indexVolt = selectVolt.find('option').length > 2 ? 0 : 1;
							var indexPlug = selectPlug.find('option').length > 2 ? 0 : 1;
							var newSRC = '';

							if (selectVolt.length) {
								var indVolt = selectVolt.find('option[seletced="selected"]').index();
								selectVolt.find('option[selected="selected"]').removeAttr('selected="selected"');
								selectVolt.prop('selectedIndex', indVolt);
							}
							if (selectPlug.length) {
								var indPlug = selectPlug.find('option[selected="selected"]').index();
								selectPlug.find('option[selected="selected"]').removeAttr('selected="selected"');
								selectPlug.prop('selectedIndex', indPlug);
							}
							
							selectVolt.find('option').each(function(i){
								var option = jQuery(this);
								if (option.text().toLowerCase().replace(/\s+/g, '') === voltage.toLowerCase().replace(/\s+/g, '')) {
									indexVolt = i;
								}
							})
							selectPlug.find('option').each(function(i){
								var option = jQuery(this);
								var optionText = option.text().toLowerCase().replace(/\s+/g, '');
								var blockText = plug.toLowerCase().replace(/\s+/g, '');
								if (optionText === blockText || optionText.indexOf(blockText) != -1) {
									indexPlug = i;
								}
							})

							if (selectVolt.length) selectVolt.prop('selectedIndex', indexVolt);
							if (selectPlug.length) selectPlug.prop('selectedIndex', indexPlug);
							...
							jQuery.ajax({
								...
								data: {
										...
										'voltage': selectVolt.val(),
										'plug': selectPlug.val()     
								},
								success: function(result){
									var newSrc = buttonSrc.split('?')[0];

									variationID.val(result);
									
									newSrc += '?add-to-cart=' + productID.val();
									newSrc += '&variation_id=' + variationID.val();
									newSrc += '&attribute_pa_voltage=' + selectVolt.val();
									newSrc += '&attribute_pa_plug=' + selectPlug.val();

									button.attr('href', newSrc);
								},
								error: function(){
									alert('ajax error on variation finding');
								}
							});
				...
				ajaxHolder.empty().css({background:''});
			}
			...
			function refreshLinks(){
				steps.eq(activeIndex).prevAll().each(function(){
					var link = jQuery(this);
					var rel = link.attr('rel');
					var hasProp = false;
					
					for(var p in dataStep){
						if (dataStep.hasOwnProperty(p)) {
							if (p === rel) {
								hasProp = true;
							}
						}
			...
			function showCurrent() {
				var id = stepsContent.eq(activeIndex).attr('id');
				var step = steps.filter('[rel="'+id+'"]');
				var currentBlock = stepsContent.eq(activeIndex);
				stepsContent.hide().eq(activeIndex).show();
				
				if (step.hasClass(hiddenClass)) {
					step = steps.eq(prevIndex);
				}
				
				steps.removeClass(activeClass+' '+allowClass);
				step.addClass(activeClass);
				step.prevAll().addClass(allowClass);
				
				// remove selected and condition classes
				currentBlock.add(currentBlock.nextAll()).find('.'+selectedClass).removeClass(selectedClass);
			};
			
			function stepsHandler(e) {
				e.preventDefault();
				var link = jQuery(this);
				if (link.hasClass(allowClass)) {
					var id = link.attr('rel');
					var block = stepsContent.filter('[id="'+id+'"]');
					...
					for(var p in dataCond){
						if (dataCond.hasOwnProperty(p)) {
							var hasProp = false;
							link.prevAll().each(function(){
								var link = jQuery(this);
								var rel = link.attr('rel');
					...
					if (emptyObject(dataStep)) {
						// if first step
						activeIndex = 0;
						activeBlock = stepsContent.eq(activeIndex).attr('id')
						jQuery.ajax({
							type: 'post',
							url: stepTarget,
							dataType: 'json',
			...
			function linksHandler(e) {
				...
				var currentBlock = currentLink.parents('.build-block');
				var key = currentBlock.attr('id');
				var value = currentLink.data('rel');
				var currentContent = currentLink.parents('.build-block');
				var currentContentName = currentContent.attr('id');
				var condition = currentLink.data('condition');

				if (currentBlock.data('name') === 'Voltage') {
					voltage = currentBlock.find('.tabset li.active a').text();
					plug = currentLink.find('h3').text();
				}
			...
			steps.on('click', stepsHandler);
			links.on('click', linksHandler);
		}
		
		var parseData = function(newHTML, cond){
			// remove old blocks
			stepList.children().not(':last').remove();
			stepContent.children().not(':last').remove();
			
			// create new blocks
			jQuery(newHTML.steps).each(function () {
				var step = this;
				var template = step.template;
				var options = jQuery(step.options);
				var stepItem = '';
				var invis = (options.length < 2 && template != 'single-option') ? ' '+hiddenClass : '';
				var hideClass = step.hidden ? hiddenClass : '';
				
				if (dataStep.step_0 === 'choice-0-1') {
					if (options.length < 2 && step.name === 'Voltage') {
						invis = '';
					}
				}
				...
				stepItem += '<li class="'+hideClass+'" rel="' + step.id + '"><a href="#" class="holder">' + step.name + '</a></li>';
				jQuery(stepItem).insertBefore(stepList.find('li:last'));
				...
				// create one-level-options template
				function createTemplate1 () {
					// create first options level
					var options = jQuery(step.options);
					var optionsStr = '';

					options.each(function () {
						var option = this;
						var descript = option.description ? '<p>' + option.description + '</p>' : '';
						var condition = option.condition.length ? 'data-condition="'+ option.condition +'"' : '';

						optionsStr += '<li><a href="#" '+condition+' data-rel="' + option.value + '">'+ '<img style="width:200px;height:200px;" src="' +
							 option.image +
							'" alt="image description">' + '<div class="text-block">'+
											'<h3>' + option.title + '</h3>'+
											descript +
										'</div>'+
									'</a></li>';
					});
					var stepBlock = '<div class="build-block'+invis+'" data-name="'+step.name+'" data-hidden="'+step.hidden+'" id="' + step.id + '">' +
								'<div class="heading">' +
									'<h2>' + step.title + '</h2>' +
								'</div>' +
								'<ul class="option-list">'+ optionsStr + '</ul>'+
							'</div>';
					jQuery(stepBlock).insertBefore(stepContent.children(':last'));
				}
				// create single-option template
				function createTemplate2 () {
					...
					options.each(function () {
						var option = this;
						var condition1 = option.condition1.length ? 'data-condition="'+ option.condition1 +'"' : '';
						var condition2 = option.condition2.length ? 'data-condition="'+ option.condition2 +'"' : '';
						optionsStr += '<li>'+ '<img style="width:200px;height:200px;" src="' +
							 option.image +
							'" alt="image description">' + '<div class="btn-holder">'+
											'<a href="#" class="btn btn-default" '+condition1+' data-rel="'+ option.value1 +'">' + option.title1 +'</a>'+
											'<a href="#" class="btn btn-grey" '+condition2+' data-rel="'+ option.value2 +'">' + option.title2 +'</a>'+
										'</div>'+
									'</li>';
					});
					var stepBlock = '<div class="build-block'+invis+'" data-name="'+step.name+'" data-hidden="'+step.hidden+'" id="' + step.id + '">' +
				...
				// create two-level-options template
				function createTemplate3 () {
					...
					options.each(function (i) {
						...
						// create children options level
						var childrens = jQuery(option.children);
						var childrenStr = '';
						
						childrens.each(function (i) {
							var children = this;
							var txtBlock = children.title ? '<div class="text-block">'+'<h3>' + children.title + '</h3>'+'</div>' : '';
							var condition = children.condition.length ? 'data-condition="'+ children.condition +'"' : '';
							
							childrenStr += '<li><a href="#" '+condition+' data-rel="'+ children.value +'">'+
								'<img style="width:200px;height:200px;" src="' + children.image + '" alt="image description">'+
									txtBlock+
							'</a></li>';
						});
						...
						if (childrens.length) {
							optionsStr += '<li class="'+ activeClass +'">'+
								'<a href="#v-' + i +'" data-toggle="tab">' + option.title + '</a>'+
							'</li>';
						}
						optionsTabStr += '<div class="tab-pane '+ activeClass +'" id="v-' + i +'">' +
										'<ul class="option-list">'+ childrenStr + '</ul>'+
									'</div>';
					});
					var stepBlock = '<div class="build-block'+invis+'" data-name="'+step.name+'" data-hidden="'+step.hidden+'" id="' + step.id + '">' +
								'<div class="heading">' +
									'<h2>' + step.title + '</h2>' +
									'<ul class="tabset">'+ optionsStr + '</ul>'+
								'</div>' +
								'<div class="tab-content">'+ optionsTabStr +'</div>'+
							'</div>';
					jQuery(stepBlock).insertBefore(stepContent.children(':last'));
				}
			});
			jQuery('.option-list').each(function (){
				var holder = jQuery(this);
				var numOfItems = holder.find('li').length;
				holder.addClass('items-count-'+ numOfItems);
			});
		}
...
{
	var bigObj=jQuery.parseHTML(newHTML);
	var commonCont=jQuery(bigObj).find('div.col-md-9.col-sm-9.col-sm-push-3');
        var OldCont=jQuery('div.col-md-9.col-sm-9.col-sm-push-3');
        var OldContOptions=jQuery('col-md-3.col-sm-3.col-sm-pull-9');
	var commonContOptions=jQuery(bigObj).find('col-md-3.col-sm-3.col-sm-pull-9');
	OldCont.empty();//commonCont.outerHTML;
	OldCont.append(commonCont);
/*	var prodHolder=commonCont.find('div.sort-holder');
	var rows=prodHolder.find('div.row');
	var lastProd=jQuery('div.sort-holder').find('div.row').last();
	rows.each(function(){
	        lastProd.append(this);
		});
	
	jQuery('div.sort-holder')*/



	OldContOptions.empty();
	OldContOptions.append(commonContOptions);

	UpdateClickEvents();
}
function getContentUrlQuery()
{
	var url=window.location+'?';
	var list=jQuery('div.widget_layered_nav');
	var firstParam=false;
	list.each( function(idx){
	    var checkedRadio=jQuery(this).find('div.rad-area.rad-checked');
			var next_obj=checkedRadio.next()[0];
			var name='';
			name=name+next_obj.name;
			var value=next_obj.value;
			name=name.replace('dropdown_layered_','');
			if(firstParam==true) url=url+'&';
			url=url+name+'='+value;
			firstParam=true;
	});
	return url;
...
{
        var urlQuery=getContentUrlQuery();
...
function initFilterForm(){
	var filterContainer = jQuery('.sort-holder');
	var wrapper = jQuery('#wrapper');
	var loadingClass = 'ajax-loading';
	var form = jQuery('.filter-form');
		form.each(function() {
			var currentForm = jQuery(this);
			var sliderContainer = currentForm.find('.ui-slider');
			var firstValue = currentForm.find('.first-value .v1');
			var lastValue = currentForm.find('.last-value .v2');
			var amountMin = currentForm.find('.amount-min');
			var amountMax = currentForm.find('.amount-max');
			var rangeMin = currentForm.find('.range-min').val();
			var rangeMax = currentForm.find('.range-max').val();


			// init slider
			function changeHandler(e, ui) {
				var values = ui ? ui.values : sliderContainer.slider('values');
				firstValue.html(values[0]);
				lastValue.html(values[1]);
				amountMin.val(values[0]);
				amountMax.val(values[1]);
			}
			...
			currentForm.on('change', ajaxFilter);
			sliderContainer.on('slidechange', ajaxFilter);
		});
}
...
function initMap() {
	...
	var myLatlng = new google.maps.LatLng(41.278221, -72.481708);
	var myOptions = {
		zoom: 13,
		center: myLatlng,
	...
	var contentString = '<div id="address-block">'+
						'<img src="images/kenyon-1.png" width="191" height="51" alt="Kenyon make any day a weekend">'+
						'<address>'+
						'PO Box 925 <br>8 Heritage Park Rd. <br>Clinton, CT 06413'+
						'</address>'+
						'</div>';
