...
function theme_load_scripts() {
	...
	if(is_checkout())
	{
		...
		wp_register_script( 'wc-checkout', get_template_directory_uri() . '/woocommerce/js/checkout.js', array( 'jquery', 'woocommerce', 'wc-country-select', 'wc-address-i18n' ), WC_VERSION, true );
...
function getAddToCartUrl($product,$voltage_data = ''){
...
add_action( 'woocommerce_order_status_cancelled', 'wc_order_cancelled');
function wc_order_cancelled($order_id) {
	
	$order = new WC_Order($order_id);
	
	if ($order->billing_email) : 
	  	$to = $order->billing_email;  // Receiver
	endif; 
	
	$from = get_option( 'woocommerce_email_from_address', true ); //Sender
    $subject = __('Your Order has been Cancelled', 'base') ;
	
	$message .= __('Dear '.$order->billing_first_name.',', 'kenyon')."\n\n";
	$message .= __('Your order '.$order_id.' has been cancelled and your payment has been voided.', 'kenyon')."\n\n";
	$message .= __('Please reply to this email if you have any questions or concerns.', 'kenyon')."\n\n";
...
// woocommerce_package_rates Filter Deprecated woocommerce_available_shipping_methods Filter in woocommerce 2.0
...
add_action( 'woocommerce_new_order', 'wc_af_check_process_queue_new' );
function wc_af_check_process_queue_new( $order_id ){

	if ( ! class_exists( 'WC_AF_Score_Helper' ) ) { return; }
	// Do the fraud check when new order is placed
		$score_helper = new WC_AF_Score_Helper();
		$score_helper->do_check( $order_id );
}

add_action( 'wp_loaded', 'wc_af_check_process_queue_existing' );
function wc_af_check_process_queue_existing(){

	if ( ! class_exists( 'WC_AF_Score_Helper' ) ) { return; }
	// Do the fraud check when the Calculate Fraud Risk button is submitted
	if ( isset( $_GET['schedule_anti_fraud'] ) && isset( $_GET['post'] ) && $_GET['post'] != '' && isset( $_GET['action'] ) && $_GET['action'] == 'edit' ) {
