...
class BuildGrill {
	...
	function ajax() {
		...
		if ($_REQUEST['ajax'] == 'steps') {
			$this->attributes = json_decode(stripslashes($_POST['jsonData']));
			echo json_encode($this->getSteps(true, $this->attributes->step_0));
			die;
		}
	...
	function getSteps($filter = true, $destination = '') {
				...
				switch($step['template']) {
					case 'one-level-options':
						foreach ($step_settings['choices'] as $choice_key => $choice_settings) {
							list($choice_image) = wp_get_attachment_image_src($choice_settings['image'], 'build-a-grill-thumbnail');
							$choice = array(
					...
					case 'single-option':
						list($choice_image) = wp_get_attachment_image_src($step_settings['image'], 'build-a-grill-thumbnail');
			...
			foreach ($result['steps'] as $step_id => $step) {
				foreach ($step['options'] as $option_id => $option) {
					if (isset($option['children'])) {
						...
						$is_marine = (isset($destination) && $destination == "choice-0-1" ); // Selected Grill is Marine Type
						
						if($is_marine){
							if ( count($result['steps'][$step_id]['options'][$option_id]['children']) < 2 && ($step_id !=9 )) { // If It is not Voltage
						...
						//if (
 							//(($is_marine && ($step_id != 'VOLTAGE_STEP_ID')) && count($result['steps'][$step_id]['options'][$option_id]['children']) < 2) //||
 							//((!$is_marine && ($step_id == 'VOLTAGE_STEP_ID')) && count($result['steps'][$step_id]['options'][$option_id]['children']) < 1)
						//){
						/*if (count($result['steps'][$step_id]['options'][$option_id]['children']) < 2)) {
							unset($result['steps'][$step_id]['options'][$option_id]);
						} else {
