...
function initRetinaCover() {
	jQuery('.bg-stretch-retina').retinaCover();
}

// paralax background init
function initParallaxBg() {
	jQuery('.parallax-section').parallaxBG({
		parent: '.bg-frame',
		image: 'img',
		parallaxOffset: 200,
		fallbackFunc: initFallbacBgStretch
	});
}

// stretch background to fill blocks
function initFallbacBgStretch() {
...
// open bootstrap dropdown on mouseover
function initBootstrapHoverDropdown(){

	var isTouchDevice = /Windows Phone/.test(navigator.userAgent) || ('ontouchstart' in window) || window.DocumentTouch && document instanceof DocumentTouch;

	jQuery('.navbar [data-toggle="dropdown"]').each(function() {
		var item = jQuery(this),
		parent = item.parent();
		if(!isTouchDevice){
...
function initCarousel() {
	jQuery('.items-slider').scrollGallery({
		mask: '.mask',
		slider: '.slideset',
		slides: '.slide',
		btnPrev: 'a.btn-prev',
	...
	jQuery('.practices-slider').scrollGallery({
		mask: '.mask',
		slider: '.slideset',
		slides: '.box',
		btnPrev: 'a.btn-prev',
...
function initCycleCarousel() {
	jQuery('.news-gallery').scrollAbsoluteGallery({
		mask: '.mask',
		slider: '.slideset',
		slides: '.slide',
		btnPrev: 'a.btn-prev',
...
function initSlideShow() {
	...
	jQuery('.quote-slideshow').fadeGallery({
		slides: '.slide',
		btnPrev: 'a.btn-prev',
		btnNext: 'a.btn-next',
		pagerLinks: '.pagination li',
...
function initSameHeight() {
	jQuery('.practice-areas-tabs').sameHeight({
		elements: 'a',
		flexible: true,
		multiLine: true,
		biggestHeight: true
...
function initCustomHover() {
	jQuery('.js-hover-item').each(function(){
		var item = jQuery(this);
		...
		item.on({
			...
			mouseover: addClassHandler,
			mouseleave: removeClassHandler
		})
	});
}
...
;(function($){
	...
	ParallaxBG.prototype = {
		...
		scrollLayout: function() {
			...
			if(!!this.ratioState) {
				if(this.parent.hasClass(this.options.alignLeftClass)) {
					var curPos = '20% ' + ((-parseFloat(this.offsetPercentage)*this.options.parallaxOffset) - (this.bgHeight - this.winHeight)/2)+'px';
				} else{
					var curPos = '50% ' + ((-parseFloat(this.offsetPercentage)*this.options.parallaxOffset) - (this.bgHeight - this.winHeight)/2)+'px';
				}
			} else {
				if(this.parent.hasClass(this.options.alignLeftClass)) {
					var curPos = '20% ' + (-parseFloat(this.offsetPercentage)*this.options.parallaxOffset)+'px';
				} else{
