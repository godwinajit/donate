  ...
  <div class="page">
    <?php 
      if (is_year()){
        ...
        $year = prev($data);
        echo '<h1 class="search-label">' . 'Yearly News Archives: ' . '</h1><h2 class="search-title">' .$year .'</h2>';
    ?>
    <?php 
      }  else {
    ?>
     <!-- Title for News -->
      <h1 class="title-center">News</h1>
    <?php 
      }
    ?>  
...
{
   query_posts( array ( 'category_name' => $_GET['category'], 'posts_per_page' => 0 ) );
			   ...
			   <?php wp_get_archives( array('post_type' => 'news','post_status' => 'publish', 'type' => 'yearly','before' => '<ul>','after' => '</ul>', 'format' => 'html', 'show_post_count' => 0 ) ); ?>
                <li> <div class="select-mobile">
                  <select name="archive-dropdown" onchange="document.location.href=this.options[this.selectedIndex].value;">
                      ...
                      <?php wp_get_archives( array('post_type' => 'news', 'type' => 'yearly', 'format' => 'option', 'show_post_count' => 1 ) ); ?>
                 </select>  
            </div></li>           
            </ul>
