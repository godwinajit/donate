...
function yamm_roots_nav_menu_args($args = '')
{
    ...
    if (!$args['items_wrap']) {
        $yamm_roots_nav_menu_args['items_wrap'] = '<div class="accordion-slide"><ul class="%2$s">%3$s</ul></div>';
    }
...
class Yamm_Fw_Nav_Walker extends Walker_Nav_Menu
{
    public function check_current($classes)
    {
        return preg_match('/(current[-_])|active|accordion-active/', $classes);
    }
    ...
    public function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0)
    {
        ...
        if ($item->is_dropdown && ($depth === 0)) {
            $item_html = str_replace('<a', '<a class="accordion-opener"', $item_html);
            $item_html = str_replace('</a>', '</a>', $item_html);
        } elseif (stristr($item_html, 'li class="divider')) {
            $item_html = preg_replace('/<a class=""[^>]*>.*?<\/a>/iU', '', $item_html);
        } elseif (stristr($item_html, 'li class="dropdown-header')) {
            $item_html = preg_replace('/<a[^>]*>(.*)<\/a>/iU', '$1', $item_html);
        }
...
{
    $slug    = sanitize_title($item->title);
    $classes = preg_replace('/(current(-menu-|[-_]page[-_])(item|parent|ancestor))/', 'active accordion-active', $classes);
    $classes = preg_replace('/^((menu|page)[-_\w+]+)+/', '', $classes);
...
function yamm_fw_roots_nav_menu_args($args = '')
{
    ...
    if (!$args['items_wrap']) {
        $yamm_fw_roots_nav_menu_args['items_wrap'] = '<div class="accordion-slide"><ul class="%2$s">%3$s</ul></div>';
    }
