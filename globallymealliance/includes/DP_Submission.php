...
function nyc_marathon_form_to_dp( $entry, $form ) {
	...
	$date_value = rgar( $entry, '4' );

	$donorDetails = saveDonor( null, null, $lastName, $email, null, null, null, $address1, null, null, null, null, null, null );
	...
	if (is_wpe()) {
		$UDFDetails = dp_save_udf_xml( $donorId, 'DOB', 'D', null, $date_value, null);
		error_log( 'nyc_marathon_form_to_dp_dob after_submission: ' . print_r( $UDFDetails, true ) );
    }else{
		$UDFDetails = dp_save_udf_xml( $donorId, 'BIRTHDATE', 'D', null, $date_value, null);
		error_log( 'nyc_marathon_form_to_dp_dob after_submission: ' . print_r( $UDFDetails, true ) );
	}
	}
}
